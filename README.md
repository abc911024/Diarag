...
| **M1** | 時序檢索 | 用 Qwen3-Embedding-0.6B 對語料做稠密向量檢索,依 M0 推斷出的年份範圍篩選候選段落,並用 coverage-aware 重排序機制,限制同一年份被選中的段落數,以增加時間分佈的多樣性。 | `build_qwen_embeddings.py`(建索引) |
| **M2** | 時序證據結構化 | 把 M1 檢索到的段落依發布年份分組,用 `gpt-oss-20b`(透過 Ollama)幫每一年生成摘要、時序訊號(positive/negative/stable/mixed/volatile/not_directly_related/no_evidence)與證據充足度判斷,組成逐年的時序證據結構。不會重新檢索。 | (見「現況」一節) |
| **M3** | 結構化時序推理 | 把 M2 產出的逐年證據結構、原始四選一選項一起丟給 `gpt-oss-20b`,選出最終答案,並回傳支持年份、證據充足度與簡短理由。 | (見「現況」一節) |
...
.
├── scripts/
│   └── run_m0_experiment_gptoss.py   # 唯一保留的 script:M0 時間範圍推斷(LLM 版,經 Ollama 呼叫 gpt-oss-20b)
...
`scripts/` 資料夾原本有 26 個檔案(M0 的兩種實作、M1/M2/M3 各版本、DB seed/export 工具、smoke test 等),經過整理後只保留 `run_m0_experiment_gptoss.py` 這一個──它不依賴 Postgres 或其他 script,單檔即可執行,直接讀 `bridge_rewrite_dataset_v3.csv` + `corpus_with_time.csv`,是最容易單獨拿出來看懂、也最容易重跑的版本。其餘刪除的檔案(Postgres 規則式 M0、M1/M2/M3 的 Postgres 版本執行腳本、DB 匯入匯出工具、smoke test)仍保留在 git 歷史紀錄中,需要的話可以從 commit log 找回來。
...
若本地服務是 OpenAI-compatible API(例如非 Ollama 的本地推論伺服器):
...
M1/M2/M3 目前沒有保留可直接執行的 script(見上「專案結構」),概念與呼叫方式請參考「四個模組」一節與最終報告的 Implementation Details。
...
- M1 使用 `Qwen/Qwen3-Embedding-0.6B` 做稠密檢索;M2、M3 使用 `gpt-oss-20b`(透過 Ollama)。這兩步都需要真的能連上 HuggingFace / Ollama 下載模型的環境與 GPU,在沒有對外網路或 GPU 的環境中無法重現。
