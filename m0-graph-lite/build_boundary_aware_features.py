...
def apply_schema(conn) -> None:
    path = Path(__file__).resolve().parent / "sql" / "006_create_boundary_aware_tables.sql"
...
def clear_existing(conn, build_version: str) -> None:
    with conn.cursor() as cur:
        cur.execute("DELETE FROM m0_graph.candidate_boundary_features WHERE build_version = %s", (build_version,))
        cur.execute("DELETE FROM m0_graph.year_trend_signals WHERE build_version = %s", (build_version,))
    conn.commit()
...
def fetch_hybrid_years(conn, m0_input_id: str, source_build_version: str) -> dict[int, dict[str, Any]]:
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """
            SELECT *
            FROM m0_graph.hybrid_query_year_evidence
            WHERE m0_input_id = %s AND build_version = %s
            """,
            (m0_input_id, source_build_version),
        )
        return {int(r["year"]): dict(r) for r in cur.fetchall()}


def fetch_hybrid_chunks(conn, m0_input_id: str, year: int, source_build_version: str) -> list[dict[str, Any]]:
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            """
...
def insert_year_trend_signal(conn, item: dict[str, Any], year: int, hybrid_row: dict[str, Any] | None, chunks: list[dict[str, Any]], build_version: str) -> None:
    signals = compute_trend_signals([c["chunk_text"] for c in chunks])
    with conn.cursor() as cur:
        cur.execute(
            """
            ...
            (
                ...
                year,
                float((hybrid_row or {}).get("hybrid_year_relevance_score") or 0.0),
                bool((hybrid_row or {}).get("year_has_hybrid_relevant_evidence") or False),
                signals["trend_signal_score"], signals["up_signal_score"], signals["down_signal_score"],
                signals["stable_signal_score"], signals["volatile_signal_score"], signals["dominant_trend_direction"],
                dump(signals["top_trend_terms"]), dump([c["chunk_id"] for c in chunks]), build_version,
            ),
        )
...
def insert_candidate_boundary_feature(conn, item: dict[str, Any], candidate: dict[str, Any], all_years: list[int], trends: dict[int, dict[str, Any]], context_size: int, build_version: str) -> None:
    ...
    contrast = compute_boundary_contrast(all_years, relevance_by_year, start, end, context_size)
...
def build_features(conn, inputs: list[dict[str, Any]], source_build_version: str, build_version: str, context_size: int) -> None:
    for item in inputs:
        ...
        hybrid_years = fetch_hybrid_years(conn, item["m0_input_id"], source_build_version)
        for year in all_years:
            chunks = fetch_hybrid_chunks(conn, item["m0_input_id"], year, source_build_version)
            insert_year_trend_signal(conn, item, year, hybrid_years.get(year), chunks, build_version)
        trends = fetch_year_trends(conn, item["m0_input_id"], build_version)
        for candidate in candidates:
            insert_candidate_boundary_feature(conn, item, candidate, all_years, trends, context_size, build_version)
        conn.commit()


def export_outputs(conn) -> None:
    exports = {
        "year_trend_signals.csv": "SELECT * FROM m0_graph.year_trend_signals ORDER BY m0_input_id, year",
        "candidate_boundary_features.csv": "SELECT * FROM m0_graph.candidate_boundary_features ORDER BY m0_input_id, candidate_window_id",
        "candidate_boundary_feature_summary.csv": "SELECT * FROM m0_graph.v_candidate_boundary_feature_summary ORDER BY rewrite_type, window_type",
    }
...
def generate_report(conn, output: Path) -> None:
    trend = fetch_dataframe(conn, "SELECT * FROM m0_graph.v_year_trend_signal_summary ORDER BY rewrite_type, build_version")
    features = fetch_dataframe(conn, "SELECT * FROM m0_graph.v_candidate_boundary_feature_summary ORDER BY rewrite_type, window_type")
    lines = [
        "# Boundary-Aware Feature Report",
        "",
        "## Purpose",
        "E6 uses hybrid year relevance as a time series and computes inside relevance, inside coherence, boundary contrast, trend signals, trend coherence, intent fit, and length fit. It is rule-based and uses no gold labels.",
        "",
        "## Formula",
        "`boundary_score_raw = 0.15 * intent_fit + 0.20 * inside_relevance_mean + 0.15 * inside_coherence + 0.20 * boundary_contrast + 0.20 * trend_coherence + 0.10 * length_fit`",
        "",
        "## Year Trend Signal Summary",
        md_table(trend),
        "",
        "## Candidate Boundary Feature Summary",
        md_table(features),
    ]
...
def print_summary(conn) -> None:
    df = fetch_dataframe(conn, """
        SELECT
            (SELECT COUNT(DISTINCT m0_input_id) FROM m0_graph.year_trend_signals) AS inputs,
            (SELECT COUNT(*) FROM m0_graph.year_trend_signals) AS years,
            COUNT(*) AS candidates,
            AVG(inside_relevance_mean) AS relevance,
            AVG(inside_coherence) AS coherence,
            AVG(boundary_contrast) AS contrast,
            AVG(trend_coherence) AS trend,
            AVG(boundary_score_raw) AS score
        FROM m0_graph.candidate_boundary_features
    """)
    row = df.iloc[0]
    print(f"number of m0 inputs processed: {int(row['inputs'] or 0)}")
    print(f"number of year trend signal rows: {int(row['years'] or 0)}")
    print(f"number of candidate boundary feature rows: {int(row['candidates'] or 0)}")
    print(f"avg inside_relevance_mean: {float(row['relevance'] or 0):.4f}")
    print(f"avg inside_coherence: {float(row['coherence'] or 0):.4f}")
    print(f"avg boundary_contrast: {float(row['contrast'] or 0):.4f}")
    print(f"avg trend_coherence: {float(row['trend'] or 0):.4f}")
    print(f"avg boundary_score_raw: {float(row['score'] or 0):.4f}")
...
def main() -> None:
    ...
    parser.add_argument("--build-version", default="boundary_v1")
    ...
    with connect_db(args) as conn:
        ...
        build_features(conn, inputs, args.source_build_version, args.build_version, args.context_size)
        export_outputs(conn)
        generate_report(conn, Path("m0-graph-lite/outputs/markdown/boundary_aware_feature_report.md"))
        print_summary(conn)
